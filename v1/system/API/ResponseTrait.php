...
trait ResponseTrait
{
    ...
    protected function respond($data = null, ?int $status = null, string $message = '')
    {
        ...
        if (is_array($dataOutput['data']) && array_key_exists('errors', $dataOutput['data']))
        {
            $dataOutput['errors'] = $dataOutput['data']['errors'];
            unset($dataOutput['data']['errors']);
        }

        if (is_null($dataOutput['data']) || empty($dataOutput['data']))
        {
            unset($dataOutput['data']);
        }

        if ($this->format === 'json') {
            return $this->response->setJSON($dataOutput)->setStatusCode($status)->setHeader('Access-Control-Allow-Origin', '*')->setHeader('Access-Control-Allow-Headers', '*')->setHeader('Access-Control-Allow-Methods', 'DELETE, GET, OPTIONS, POST, PUT');
        }

        if ($this->format === 'xml') {
            return $this->response->setXML($dataOutput)->setStatusCode($status);
        }

        return $this->response->setBody($dataOutput)->setStatusCode($status);
    }
